...
{
    public partial class AddNewTriggerForApprovalLevelDetailsAuditTrailDelete : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_ApprovalLevelDetailAuditTrailDelete] ON [dbo].[ApprovalLevelDetails] FOR DELETE
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'ApprovalLevelDetails'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @ApprovalLevelDetailId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@ApprovalLevelDetailId = Id
			From
				#DELETED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@ApprovalLevelDetailId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End";
        ...
        {
			string trigger = @"Drop Trigger Trig_ApprovalLevelDetailAuditTrailDelete";
			migrationBuilder.Sql(trigger);
		}
    }
}
