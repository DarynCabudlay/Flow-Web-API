...
{
    public partial class AddNewTriggerForRequestTypeWorkFlowAuditTrailInsert : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestTypeWorkFlowAuditTrailInsert] ON [dbo].[RequestTypeWorkFlows] FOR INSERT
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'RequestTypeWorkFlows'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @RequestTypeWorkFlowId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@RequestTypeWorkFlowId = Id
			From
				#INSERTED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@RequestTypeWorkFlowId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
            string trigger = @"Drop Trigger Trig_RequestTypeWorkFlowAuditTrailInsert";
            migrationBuilder.Sql(trigger);
        }
    }
}
