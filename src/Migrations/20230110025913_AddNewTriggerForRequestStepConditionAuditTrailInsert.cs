...
{
    public partial class AddNewTriggerForRequestStepConditionAuditTrailInsert : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestStepConditionAuditTrailInsert] ON [dbo].[RequestStepConditions] FOR INSERT
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'RequestStepConditions'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @RequestStepConditionId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@RequestStepConditionId = Id
			From
				#INSERTED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@RequestStepConditionId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_RequestStepConditionAuditTrailInsert";
			migrationBuilder.Sql(trigger);
		}
    }
}
