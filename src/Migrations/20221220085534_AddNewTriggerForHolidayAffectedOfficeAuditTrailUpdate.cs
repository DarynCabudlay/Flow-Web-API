...
{
    public partial class AddNewTriggerForHolidayAffectedOfficeAuditTrailUpdate : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_HolidayAffectedOfficeAuditTrailUpdate] ON [dbo].[HolidayAffectedOffices] FOR UPDATE
NOT FOR REPLICATION
AS
BEGIN
	...
	SET @TableName = 'HolidayAffectedOffices'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @HolidayOfficeId as int
						...
						Set @SQLQuery = '
								...
								(
									SELECT
										...
										' + CAST(@HolidayOfficeId AS VARCHAR(10)) + ',
										' + '''' + @TableName + '''' + ',
										' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_HolidayAffectedOfficeAuditTrailUpdate";
			migrationBuilder.Sql(trigger);
		}
    }
}
