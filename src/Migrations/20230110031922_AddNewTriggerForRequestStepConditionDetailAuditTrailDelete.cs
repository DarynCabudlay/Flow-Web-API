...
{
    public partial class AddNewTriggerForRequestStepConditionDetailAuditTrailDelete : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestStepConditionDetailAuditTrailDelete] ON [dbo].[RequestStepConditionDetails] FOR DELETE
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'RequestStepConditionDetails'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @RequestStepConditionDetailId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@RequestStepConditionDetailId = Id
			From
				#DELETED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@RequestStepConditionDetailId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_RequestStepConditionDetailAuditTrailDelete";
			migrationBuilder.Sql(trigger);
		}
    }
}
