...
{
    public partial class AddNewTriggerForUserOrganizationalStructuresAuditTrailDelete : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_UserOrganizationalStructureAuditTrailDelete] ON [dbo].[UserOrganizationalStructures] FOR DELETE
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'UserOrganizationalStructures'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @UserOrganizationalStructureId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@UserOrganizationalStructureId = Id
			From
				#DELETED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@UserOrganizationalStructureId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End";
        ...
        {
			string trigger = @"Drop Trigger Trig_UserOrganizationalStructureAuditTrailDelete";
			migrationBuilder.Sql(trigger);
		}
    }
}
