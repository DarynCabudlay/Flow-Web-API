...
{
    public partial class AddNewTriggerForRequestStepAssigneeDetailAuditTrailInsert : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestStepAssigneeDetailAuditTrailInsert] ON [dbo].[RequestStepAssigneeDetails] FOR INSERT
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'RequestStepAssigneeDetails'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @RequestStepAssigneeDetailId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@RequestStepAssigneeDetailId = Id
			From
				#INSERTED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@RequestStepAssigneeDetailId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_RequestStepAssigneeDetailAuditTrailInsert";
			migrationBuilder.Sql(trigger);
		}
    }
}
