...
{
    public partial class AddNewTriggerForRequestStepAssigneeAuditTrailUpdate : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestStepAssigneeAuditTrailUpdate] ON [dbo].[RequestStepAssignees] FOR UPDATE
NOT FOR REPLICATION
AS
BEGIN
	...
	SET @TableName = 'RequestStepAssignees'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @RequestStepAssigneesId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@RequestStepAssigneesId = Id
			From
				#INSERTED
			Where
				RowNum = @MinRowNum
						...
						Set @SQLQuery = '
								...
								(
									SELECT
										...
										' + CAST(@RequestStepAssigneesId AS VARCHAR(10)) + ',
										' + '''' + @TableName + '''' + ',
										' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_RequestStepAssigneeAuditTrailUpdate";
			migrationBuilder.Sql(trigger);
		}
    }
}
