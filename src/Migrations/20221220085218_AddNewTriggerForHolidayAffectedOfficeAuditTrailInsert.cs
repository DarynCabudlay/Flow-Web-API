...
{
    public partial class AddNewTriggerForHolidayAffectedOfficeAuditTrailInsert : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_HolidayAffectedOfficeAuditTrailInsert] ON [dbo].[HolidayAffectedOffices] FOR INSERT
NOT FOR REPLICATION
AS
Begin
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@HolidayOffceId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_HolidayAffectedOfficeAuditTrailInsert";
			migrationBuilder.Sql(trigger);
		}
    }
}
