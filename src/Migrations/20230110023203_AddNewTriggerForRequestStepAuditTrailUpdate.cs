...
{
    public partial class AddNewTriggerForRequestStepAuditTrailUpdate : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestStepAuditTrailUpdate] ON [dbo].[RequestSteps] FOR UPDATE
NOT FOR REPLICATION
AS
BEGIN
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			While @I <= @Num_Cols BEGIN
				...
				If @ColName NOT IN ('RequestTypeId', 'Version', 'StepId') BEGIN
					...
					If (SUBSTRING(COLUMNS_UPDATED(),(@ColumnId - 1) / 8 + 1, 1)) & POWER(2, (@ColumnId - 1) % 8) = POWER(2, (@ColumnId - 1) % 8) Begin
						...
						Set @SQLQuery = '
								...
								(
									...
									FROM
										#DELETED D
										LEFT JOIN #INSERTED I ON I.RequestTypeId = D.RequestTypeId AND I.Version = D.Version AND I.StepId = D.StepId
									WHERE
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_RequestStepAuditTrailUpdate";
			migrationBuilder.Sql(trigger);
		}
    }
}
