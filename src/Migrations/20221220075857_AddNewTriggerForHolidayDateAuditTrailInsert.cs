...
{
    public partial class AddNewTriggerForHolidayDateAuditTrailInsert : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_HolidayDateAuditTrailInsert] ON [dbo].[HolidayDates] FOR INSERT
NOT FOR REPLICATION
AS
Begin
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@HolidayDateId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_HolidayDateAuditTrailInsert";
			migrationBuilder.Sql(trigger);
		}
    }
}
