...
{
    public partial class AddNewTriggerForRequestStepAssigneeAuditTrailDelete : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_RequestStepAssigneeAuditTrailDelete] ON [dbo].[RequestStepAssignees] FOR DELETE
NOT FOR REPLICATION
AS
Begin
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @RequestStepAssigneeId as int
			Declare @TrailDate as Datetime = Getdate()

			Select
				@RequestStepAssigneeId = Id
			From
				#DELETED
			Where
				RowNum = @MinRowNum
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@RequestStepAssigneeId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_RequestStepAssigneeAuditTrailDelete";
			migrationBuilder.Sql(trigger);
		}
    }
}
