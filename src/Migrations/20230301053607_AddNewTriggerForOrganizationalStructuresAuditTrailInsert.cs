...
{
    public partial class AddNewTriggerForOrganizationalStructuresAuditTrailInsert : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_OrganizationalStructureAuditTrailInsert] ON [dbo].[OrganizationalStructures] FOR INSERT
NOT FOR REPLICATION
AS
Begin
	...
	SET @TableName = 'OrganizationalStructures'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @OrganizationalStructureId as int
					...
					Set @SQLQuery = '
							...
							(
								SELECT
									...
									' + CAST(@OrganizationalStructureId AS VARCHAR(10)) + ',
									' + '''' + @TableName + '''' + ',
									' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_OrganizationalStructureAuditTrailInsert";
			migrationBuilder.Sql(trigger);
		}
    }
}
