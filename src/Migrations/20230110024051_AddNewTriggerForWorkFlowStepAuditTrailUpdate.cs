...
{
    public partial class AddNewTriggerForWorkFlowStepAuditTrailUpdate : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_WorkFlowStepAuditTrailUpdate] ON [dbo].[WorkFlowSteps] FOR UPDATE
NOT FOR REPLICATION
AS
BEGIN
						...
						Set @SQLQuery = '
								...
								(
									SELECT
										...
										' + CAST(@WorkFlowStepId AS VARCHAR(10)) + ',
										' + '''' + @TableName + '''' + ',
										' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_WorkFlowStepAuditTrailUpdate";
			migrationBuilder.Sql(trigger);
		}
    }
}
