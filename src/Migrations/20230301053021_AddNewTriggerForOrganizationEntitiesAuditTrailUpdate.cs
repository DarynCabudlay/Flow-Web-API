...
{
    public partial class AddNewTriggerForOrganizationEntitiesAuditTrailUpdate : Migration
    {
...
GO
CREATE TRIGGER [dbo].[Trig_OrganizationEntityAuditTrailUpdate] ON [dbo].[OrganizationEntities] FOR UPDATE
NOT FOR REPLICATION
AS
BEGIN
	...
	SET @TableName = 'OrganizationEntities'
	...
	If @MaxRowNum > 0 Begin
		...
		While @MinRowNum <= @MaxRowNum Begin
			...
			Declare @OrganizationEntityId as int
						...
						Set @SQLQuery = '
								...
								(
									SELECT
										...
										' + CAST(@OrganizationEntityId AS VARCHAR(10)) + ',
										' + '''' + @TableName + '''' + ',
										' + '''' + @UserId + '''' + ',
...
End
        ...
        {
			string trigger = @"Drop Trigger Trig_OrganizationEntityAuditTrailUpdate";
			migrationBuilder.Sql(trigger);
		}
    }
}
